    ...
    while not rospy.is_shutdown():
        ...
        for id, index, preds, x1, y1, x2, y2 in boxes:
            if index == 39:
                ...
                cv2.putText(frame, str(cnt//10), (x1, y1-30), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 0), 5)
        ...
        if pose is not None:
            pose_draw()
            for id_, index, conf, x1, y1, x2, y2 in boxes:
                if(index == 39):
            ...
            for id_, index, conf, x1, y1, x2, y2 in boxes:
                if(index == 39):
                    if id_ == flag:
                        cv2.putText(frame, str(int(cnt)//10), (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
